...
public static class AirlineLogoChecker
{
    public static async Task CheckAsync(WebApplication app)
    {
        ...
        Console.WriteLine("\n[2] DB'deki aktif airline'lar:");
        var airlines = await db.Airlines
            .Where(a => a.IsActive)
            .OrderBy(a => a.Code)
            .Select(a => new { a.Code, a.NameTr, a.LogoUrl })
            .ToListAsync();

        foreach (var a in airlines)
            Console.WriteLine($"  {a.Code} - {a.NameTr} (LogoUrl: {a.LogoUrl ?? "NULL"})");

        Console.WriteLine($"  Toplam: {airlines.Count} aktif airline");
