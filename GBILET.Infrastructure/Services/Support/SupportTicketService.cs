...
public class SupportTicketService : ISupportTicketService
{
        ...
        CancellationToken ct = default)
    {
        return await _db.SupportTickets
            ...
            .Select(t => new SupportTicketListItemDto
            {
                ...
                BookingPnr = t.Booking != null ? (t.Booking.InternalPnr ?? t.Booking.PNR) : null,
                UserId = t.UserId,
        ...
        CancellationToken ct = default)
    {
        ...
        var ticket = new SupportTicket
        {
            ...
            Type = request.Type,
            Subject = request.Subject.Trim(),
            GuestEmail = string.IsNullOrWhiteSpace(request.GuestEmail) ? null : request.GuestEmail.Trim().ToLowerInvariant(),
            Status = SupportTicketStatus.Open,
            CreatedAt = now,
            UpdatedAt = now,
            LastActivityAt = now
    ...
    private async Task<SupportTicketDetailDto?> LoadDetailAsync(Guid ticketId, CancellationToken ct)
    {
        ...
        return new SupportTicketDetailDto
        {
            ...
            BookingPnr = ticket.Booking?.InternalPnr ?? ticket.Booking?.PNR,
